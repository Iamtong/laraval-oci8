...
namespace Iamtong\Oci8\Pdo;

use PDO;
use Iamtong\Oci8\Pdo\Oci8\Statement;
use Iamtong\Oci8\Pdo\Oci8\Exceptions\Oci8Exception;
...
class Oci8 extends PDO
{
    ...
    public function prepare($statement, $options = null)
    {
        ...
        //处理不兼容的函数
        $statement = $this->TransformDateFormat($statement);
     ...
     * Iamtong\Oci8\Pdo\Oci8\Statement object.
     *
     * @param string $statement The SQL statement to prepare and execute.
     ...
     * @param $sql
     */
    public function TransformDateFormat($sql){
        //处理AS统一去掉
        $sql = str_ireplace(' AS ', ' ', $sql);
        //去掉mysql的··
        ...
        //位运算处理
        $sql = preg_replace('/\s*(\w+)\s*&\s*([0-9]+)\s*?=/',' BITAND($1,$2)=',$sql);
        $sql = preg_replace('/\s*(\w+)\s*\|\s*([0-9]+)\s*?=/',' BITOR($1,$2)=',$sql);
        $sql = preg_replace('/\s*(\w+)\s*\^\s*([0-9]+)\s*?=/',' BITXOR($1,$2)=',$sql);

        return $sql;
    }

    function date_replace($matches){
        $date_dict_arr = [
            '%y' => 'yy',
            '%Y' => 'yyyy',
            '%m' => 'MM',
        ...
        $re_str = str_ireplace('DATE_FORMAT(','TO_CHAR(',$matches[0]);
        $re_str = str_ireplace(array_keys($date_dict_arr),array_values($date_dict_arr),$re_str);
        return $re_str;
    }
}
