...
namespace Iamtong\Oci8\Query\Processors;
...
class OracleProcessor extends Processor
{
     ...
     * @param Builder $query
     * @param string $sql
     * @return \PDOStatement|\Iamtong\Oci8\Pdo\Oci8
     */
    private function prepareStatement(Builder $query, $sql)
    {
        /** @var \Iamtong\Oci8\Oci8Connection $connection */
        $connection = $query->getConnection();
        $pdo        = $connection->getPdo();

        if (strpos(strtolower($sql), ' as ') !== false) {
    ...
    protected function incrementBySequence(array $values, $sequence)
    {
        ...
        if (! isset($builderArgs[1][0][$sequence])) {
            if ($builder instanceof EloquentBuilder) {
                /** @var \Iamtong\Oci8\Eloquent\OracleEloquent $model */
